...
exports.getArtisanById = async (req, res) => {
  try {
    ...
    res.json({ profile, products: products || [] });
  } catch (err) {
...
exports.updateMyProfile = async (req, res) => {
  try {
    const { store_name, artisan_type, specialization, location, bio, profile_image, preferred_language } = req.body;
    const { data, error } = await supabase
      .from('artisan_profiles')
      .update({ store_name, artisan_type, specialization, location, bio, profile_image, preferred_language })
      .eq('user_id', req.user.id)
      .select()
      .single();
    if (error) throw error;
